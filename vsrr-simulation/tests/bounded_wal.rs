...
use vsrr_proto::OpNumber;
use vsrr_simulation::{CheckResult, Cluster, DurabilityChecker, Faults, check_safety};
...
fn primary_wal_window_is_intact(c: &Cluster, primary: usize) -> Result<(), String> {
  ...
  for op in (ckpt + 1)..=head {
    if !c.replica_wal_holds_op(primary, vsrr_proto::OpNumber::with(op)) {
      return Err(format!(
