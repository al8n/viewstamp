...
impl StateMachine for SharedSm {
  ...
  fn restore(&mut self, snapshot: &[u8]) {
    self.0.borrow_mut().restore(snapshot)
  }
}
...
/// Bind a fresh 3-node cluster on `127.0.0.1` TCP ports starting at `base_port` over the plain
...
/// alongside each node's shared [`BatchSm`] recorder. Distinct ports per test keep
/// concurrently-running tests from colliding on the loopback address.
async fn spawn_cluster(
  base_port: u16,
) -> (Vec<viewstamp_compio::Handle>, Vec<Rc<RefCell<BatchSm>>>) {
...
async fn concurrent_units_batch_into_fewer_ops_over_a_real_cluster() {
  let (mut handles, sms) = spawn_cluster(45850).await;
...
async fn a_generous_stall_deadline_never_fires_on_a_healthy_cluster() {
  let (mut handles, _sms) = spawn_cluster(45860).await;
