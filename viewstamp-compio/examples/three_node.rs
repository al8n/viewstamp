...
//! ```
//!
//! No extra feature flags are needed: `viewstamp-compio` has no cargo features of its own and
//! already enables what it needs from `viewstamp-proto` (`std`, `quic`, `tls-rustls-ring`).
//!
...
async fn main() {
  ...
  let addrs: Vec<SocketAddr> = (0..3)
    .map(|i| format!("127.0.0.1:{}", 47100 + i).parse().unwrap())
    .collect();
  ...
  for id in 0u16..3 {
    ...
    // anything else.
    let mk_dialer: Rc<dyn Fn(Peer) -> Conn<Labeled<Passthrough>>> = {
      let opts = LabelOptions::new(CLUSTER, Peer::Replica(ReplicaId::new(id)));
      Rc::new(move |_peer| Conn::from_parts(Labeled::dialer(Passthrough::new(), &opts)))
    };
    let mk_acceptor: Rc<dyn Fn() -> Conn<Labeled<Passthrough>>> = {
      let opts = LabelOptions::new(CLUSTER, Peer::Replica(ReplicaId::new(id)));
      Rc::new(move || Conn::from_parts(Labeled::acceptor(Passthrough::new(), &opts)))
    };
    ...
    // One driver task per replica; the returned `Handle` is the application's way in.
