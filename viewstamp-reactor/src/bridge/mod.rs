...
pub(crate) struct DialReady<R: Runtime> {
  ...
  /// The outbound receiver the writer drains (moved into the dial task in `dial_peer`, shipped
  /// back here on completion so the bridge — not the dial task — owns it).
...
pub(crate) enum ConnTask<R: Runtime> {
  /// The dial task, until it succeeds.
  #[allow(dead_code)]
  Connecting(AbortOnDrop<R>),
  ...
  /// the two tasks make progress independently. Either task signalling EOF/error leads the driver
  /// to `close_conn`, which drops this `Bridged` and so aborts the OTHER half.
  #[allow(dead_code)]
  Bridged {
    read: AbortOnDrop<R>,
    write: AbortOnDrop<R>,
...
/// the `tasks` [`AbortOnDrop`] handle(s) abort the live task(s) (dial OR both bridge halves) —
/// preempting a stuck write and releasing the socket halves — and `out_tx`'s drop signals a
/// still-running writer to close.
pub(crate) struct Conn<R: Runtime> {
...
/// half from reading: the two halves make progress independently. On `Ok(0)` the peer closed its
/// write side (EOF); on `Err` the socket failed. Either ends this half with the matching inbound
/// signal, and the driver's `close_conn` then drops the whole `Conn` — aborting the writer half (for
/// a dialed conn, the close path also redials).
pub(crate) async fn bridge_read(
  mut read_half: impl AsyncRead + Unpin,
  id: ConnId,
  inbound_tx: flume::Sender<BridgeInbound>,
) {
  // One buffer for the task's lifetime: a readiness read borrows it per call, so every iteration
  // re-lends the same allocation instead of zeroing a fresh 64 KiB per read. Only the
  // exactly-`n`-sized inbound copy is allocated per chunk (the channel needs an owned `Vec`; an
  // oversize forward would carry the buffer's stale tail).
...
/// progress independently (the full-duplex stall a single read+write task suffers under large
/// bidirectional traffic cannot happen here).
///
/// Each chunk drains through a plain sequential write loop whose `written` cursor is the only
/// partial-I/O state — task-local and outside any select, so no lost wake can desynchronize it —
/// and `queued_bytes` is released INCREMENTALLY, by the count actually written on each partial
/// write, so the driver's enqueue-side byte budget reflects what is still in flight WHILE a large
/// chunk is still draining, not only once it finishes (a chunk mid-drain must not keep the counter
/// over the cap and false-close a healthy conn). The decrements for one chunk always sum to exactly
/// its length (no under/over-count, no underflow): on a terminal short-write/error the REMAINING
/// unwritten bytes are released. Dropping `out_tx` (the driver dropped this conn's `Conn`) drains
/// the FIFO, then ends this half with `Eof`; an I/O error ends it with `Error`. A HARD abort (a
/// stuck write the graceful close can't preempt) is external: the driver drops this task's
/// [`AbortOnDrop`], which aborts the task on every runtime.
