...
//! deterministic per-unit function. The stall-smoke companion pins the `Send` sleep-factory wiring
//! (`tokio::time::sleep` across `tokio::spawn`) and that a generous deadline never fires on a
//! healthy run.
...
/// value while the test keeps an `Arc` clone of the inner [`BatchSm`] to read its per-unit apply
/// history — the cluster-side witness that units coalesced into fewer consensus ops. The lock (not
/// a `RefCell`) keeps the wrapper `Send`, which `tokio::spawn` requires of the driver's `run()`
/// future.
struct SharedSm(Arc<Mutex<BatchSm>>);

impl SharedSm {
  fn new() -> (Self, Arc<Mutex<BatchSm>>) {
    let inner = Arc::new(Mutex::new(BatchSm::default()));
...
/// The concrete driver under test: pinned to the tokio `agnostic` runtime and the plain record
/// layer (batching is record-layer-independent — the TLS swap is the stream gate's concern), over
/// [`SharedSm`] and [`Notifying`] throwaway storage.
type GateDriver = viewstamp_reactor::ReactorStreamDriver<
...
fn mk_acceptor(me: u8) -> Arc<dyn Fn() -> Conn<Labeled<Passthrough>> + Send + Sync> {
  Arc::new(move || {
    let opts = LabelOptions::new(CLUSTER, Peer::Replica(ReplicaId::new(me as u16)));
...
/// the aggregator. Runs on the multi-thread flavor so the spawned pump and submit tasks can
/// migrate across workers — the plain pump's `run()` future is `Send` (the no-stall sleep factory
...
async fn concurrent_units_batch_into_fewer_ops_over_a_real_cluster() {
  ...
  let group_task = {
    ...
    tokio::spawn(async move { batch.submit_group(units).await.expect("the group commits") })
  };
  ...
  let group_replies = tokio::time::timeout(Duration::from_secs(15), group_task)
    .await
  ...
  wait_until(
    10,
    || sms[0].lock().unwrap().units().len() >= 16,
    "replica 0 to apply all 16 units",
  )
  .await;
  let applied: Vec<(u64, u32, Bytes)> = sms[0].lock().unwrap().units().to_vec();
  ...
  drop(batch);
  tokio::time::timeout(Duration::from_secs(5), pump_task)
    .await
...
/// The stall-smoke + the positive `Send` pin: [`aggregator_with_stall`] with tokio's sleep — a
/// `Send` factory minting `Send` futures — yields a pump whose `run()` crosses `tokio::spawn`
/// (the compile-time pin), and a GENEROUS deadline never fires on a healthy cluster: every reply
/// resolves Ok (a stall would resolve them `OutcomeUnknown`/`Refused` instead) and the pump still
/// exits through the drained-queue path at teardown. The terminal-stall BEHAVIOR is unit-tested in
