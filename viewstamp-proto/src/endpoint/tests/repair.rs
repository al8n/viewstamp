...
fn a_misplaced_repaired_prepare_is_rejected_not_adopted() {
  ...
  r.handle_message(now, &mut storage, primary_peer(), repair_prepare(0, 5, 3));
  assert_eq!(
    r.commit(),
    OpNumber::with(1),
...
fn fill_repair_rejects_a_stale_uncommitted_prepare_for_a_committed_hole() {
  ...
  r.handle_message(now, &mut storage, primary_peer(), repair_prepare(0, 2, 1));
  assert_eq!(
    r.commit(),
    OpNumber::with(1),
...
fn repair_holds_the_commit_across_a_long_unrepaired_window() {
  ...
  let (mut r, mut storage) = recovering_with_hole(4, 2);
  let mut blocks = crate::block_store::InMemoryBlockStore::new();
  while r.poll_message().is_some() {}
