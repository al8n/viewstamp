...
fn foreign_epoch_prepare_ok_does_not_count_toward_the_vote_quorum() {
  ...
  e.storage_step(now, &mut storage, &mut blocks); // own append durable → own vote (bit 0)
...
fn same_config_prepare_happy_path_still_appends_and_acks() {
  ...
  e.storage_step(now, &mut storage, &mut blocks); // pump WAL → PrepareOk
  match e
    .poll_message()
...
fn foreign_epoch_prepare_normal_arm_is_dropped_but_repair_arm_is_agnostic() {
  ...
  let (mut r, mut storage) = recovering_with_hole_at_epoch(3, 2, 1);
  let mut blocks = crate::block_store::InMemoryBlockStore::new();
  assert_eq!(
    r.membership.epoch(),
    Epoch::new(1),
  ...
  let (mut e, _storage2) = recovering_with_hole_at_epoch(3, 2, 1);
...
fn a_pre_transition_appends_completion_is_absorbed_while_recovering() {
  ...
  assert_eq!(
    ...
    1,
    "the append is physically in flight"
  );
  assert!(e.has_inflight_storage(&storage));
  while e.poll_message().is_some() {}
...
fn a_pending_durable_view_write_defers_the_cross_epoch_peer_fetch() {
  ...
  e.handle_timeout(later, &mut storage); // primary_idle due → own SVC(view 1)
  e.handle_message(
    later,
    &mut storage,
    Peer::Replica(ReplicaId::new(2)),
