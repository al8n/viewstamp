  ...
  R: StreamTransport,
{
  ...
  /// materialize occupies the lane rather than the thread that pumps consensus. The caller owns the
  /// [`BlockJobCursor`](crate::BlockJobCursor) and with it the serial-in-issue-order obligation the
  /// inline lane satisfies by construction.
  ...
  /// The tail every storage-advancing entry point shares: reconcile routing against a membership the
